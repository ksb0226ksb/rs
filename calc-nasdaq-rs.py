...
print("Fetching NASDAQ stock list...")
...
def calc_score(data, day=-1):
    try:
        ...
        print(f"Not enough data available. {e}")
        return -1
...
with open(result_file_path, "w") as f:
    ...
    comment = '''\
    Calculated Relative Strength ratings for all NASDAQ stocks.
...
with open(result_file_path, "w") as f:
    ...
    comment = '''\
    Calculated Mark Minervini's Trend Template and listed only stocks that meet the criteria. Stocks that don't pass the filter are not listed.

    Uses the Minervini Trend Template calculation method described below. The calculation requires RS values of at least 70 (80 and 90 are better), but only shows results with 70 or above without special highlighting for 80 or 90.
    ...
    f.write("\n")
    footer = '''\
    ## Minervini Trend Template Calculation
